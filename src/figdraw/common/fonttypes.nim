...
when defined(figdrawNativeDynlib):
  type ArrangementRunes = seq[Rune]
else:
  type
...
type
  GlyphArrangement* = object
    ...
    sourceRunes*: ArrangementRunes ## Source runes for glyph source ranges.
    arrangedGlyphs*: seq[ArrangedGlyph] ## Glyph-id-first placement data.
    runes*: ArrangementRunes
      ## Display runes, one for each visual glyph where applicable.
...
when not defined(figdrawNativeDynlib):
