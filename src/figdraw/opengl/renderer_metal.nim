...
type OpenGLRenderer* = ref object
  ## Despite the name, this becomes a Metal-backed renderer when
  ## `-d:feature.figdraw.metal` is enabled on macOS.
  ctx*: Context
...
proc newOpenGLRenderer*(ctx: Context): OpenGLRenderer =
  ## Uses a caller-created Metal Context (for example, with a presentLayer set).
